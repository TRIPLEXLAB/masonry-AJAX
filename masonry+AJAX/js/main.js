$(function () {
    ...
    $('#gallery').each(function () { //이미지를 반복적으로 불러오기
        ...
        //형식) $(선택자).masony(옵션)=>그리드 형태로 배치
        $container.masonry({
            //columnWidth: 100, /* 열의 길이(필수지정)*/
            //gutter: 10, //열 사이의 좌우간격
            horizontalOrder: true,
            itemSelector: '.gallery-item' //그리드에 해당하는 요소선택자를 지정속성
        });
		/*{
        ...
        $.getJSON('./data/content.json', function (data) {
            var elements = [];//태그포함한 이미지 문자열배열 생성
            //var elements={}; //빈객체
            $.each(data, function (i, item) {//i 인덱스번호, item(각 이미지 정보태그)
                var itemHTML = '<li class="gallery-item is-loading">' +
                    ...
                    '</a>' + '</li>';
                //html문자열을 배열에 저장
                elements.push($(itemHTML).get(0));//현재 읽어들여서 집어대상자
            })
            //ul태그에 li태그를 추가->부모.append(자식)
            $container.append(elements)//배열추가
            //이미지가 메모리에 다 불러오고나서 길이가 다 다른 이미지->그리드형태
            //이미지를 다 불러왔다면 is-loading클래스 선택자해제->화면에 보인다.
